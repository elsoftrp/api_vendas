...
class UnidadeController extends Controller
{
    protected $model;
    protected $user;
    protected $nomeprograma = 'Unidades';

    public function __construct(Unidade $unidade, User $user)
    {
        $this->model = $unidade;
        $this->user  = $user;
    }
    ...
    public function seek(Request $request)
    {
        ...
        if ($direitos)
        {
            ...
            $data = DB::table('grupos')->select(DB::raw("id, desunidade, unidade"))
            ->where('unidade','=', $valorPesquisa)
            ->orWhere('desunidade','=',$valorPesquisa)
            ->limit(10)
            ->get();
            return $data;
        } else
